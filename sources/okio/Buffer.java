...
public final class Buffer implements BufferedSource, BufferedSink, Cloneable, ByteChannel {
    ...
    public short readShort() {
        if (this.size >= 2) {
            ...
            int limit = segment.limit;
            if (limit - pos < 2) {
                return (short) (((readByte() & UnsignedBytes.MAX_VALUE) << 8) | (readByte() & 255));
            }
            byte[] data = segment.data;
            int pos2 = pos + 1;
            int pos3 = pos2 + 1;
            int s = ((data[pos] & UnsignedBytes.MAX_VALUE) << 8) | (data[pos2] & 255);
            this.size -= 2;
            if (pos3 == limit) {
                this.head = segment.pop();
                SegmentPool.recycle(segment);
    ...
    public int readInt() {
        if (this.size >= 4) {
            ...
            int pos2 = pos + 1;
            int pos3 = pos2 + 1;
            int i = ((data[pos] & UnsignedBytes.MAX_VALUE) << Ascii.CAN) | ((data[pos2] & UnsignedBytes.MAX_VALUE) << Ascii.DLE);
            int pos4 = pos3 + 1;
            int i2 = i | ((data[pos3] & UnsignedBytes.MAX_VALUE) << 8);
            int pos5 = pos4 + 1;
    ...
    private ByteString digest(String algorithm) {
        try {
            ...
            return ByteString.m601of(messageDigest.digest());
    ...
    private ByteString hmac(String algorithm, ByteString key) {
        try {
            ...
            return ByteString.m601of(mac.doFinal());
        } catch (NoSuchAlgorithmException e) {
            throw new AssertionError();

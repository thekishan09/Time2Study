...
public class TypefaceCompatApi29Impl extends TypefaceCompatBaseImpl {
    ...
    public Typeface createFromFontFamilyFilesResourceEntry(Context context, FontResourcesParserCompat.FontFamilyFilesResourceEntry familyEntry, Resources resources, int style) {
        ...
        int i4 = (style & 1) != 0 ? LogSeverity.ALERT_VALUE : LogSeverity.WARNING_VALUE;
        if ((style & 2) != 0) {
            i = 1;
        }
    ...
    public Typeface createFromResourcesFontFile(Context context, Resources resources, int id, String path, int style) {
        try {
            return new Typeface.CustomFallbackBuilder(new FontFamily.Builder(new Font.Builder(resources, id).build()).build()).setStyle(new FontStyle((style & 1) != 0 ? LogSeverity.ALERT_VALUE : LogSeverity.WARNING_VALUE, (style & 2) != 0 ? 1 : 0)).build();
        } catch (IOException e) {
            return null;
        }
    }

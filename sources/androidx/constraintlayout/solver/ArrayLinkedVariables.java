...
public class ArrayLinkedVariables {
        ...
        if (Math.abs(value) >= epsilon) {
            ...
            while (current != -1 && counter < this.currentSize) {
                if (this.mArrayIndices[current] == variable.f32id) {
                    ...
                    if (Math.abs(fArr[current]) < epsilon) {
    ...
    public final void updateFromRow(ArrayRow self, ArrayRow definition, boolean removeFromDefinition) {
        ...
        int counter = 0;
        while (current != -1 && counter < this.currentSize) {
            if (this.mArrayIndices[current] == definition.variable.f32id) {
    ...
    public final int getVariable(PriorityGoalRow.RowVariable rowVariable, int index) {
        rowVariable.value = this.mArrayValues[index];
        rowVariable.variableId = this.mArrayIndices[index];
        return this.mArrayNextIndices[index];
    }

    public ArrayList<PriorityGoalRow.RowVariable> getVariables() {
        ArrayList<PriorityGoalRow.RowVariable> variablesList = new ArrayList<>();
        int current = this.mHead;
        int counter = 0;
        while (current != -1 && counter < this.currentSize) {
            PriorityGoalRow.RowVariable rowVariable = new PriorityGoalRow.RowVariable();
            rowVariable.value = this.mArrayValues[current];
            rowVariable.variableId = this.mArrayIndices[current];
            variablesList.add(rowVariable);
            current = this.mArrayNextIndices[current];
            counter++;
        }
    ...
    public void updateFromSystem(ArrayRow self, ArrayRow[] rows) {
        ...
        while (current != -1 && counter < this.currentSize) {
            ...
            if (variable.definitionId != -1) {
                ...
                self.constantValue += definition.constantValue * value;
                if (Math.abs(self.constantValue) < epsilon) {
                    self.constantValue = 0.0f;
                }
                definition.variable.removeFromRow(self);
                current = this.mHead;
                counter = 0;

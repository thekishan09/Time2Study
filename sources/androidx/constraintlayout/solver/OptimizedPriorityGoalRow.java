...
public class OptimizedPriorityGoalRow extends ArrayRow {
    ...
    class GoalVariableAccessor implements Comparable {
        ...
        public GoalVariableAccessor(OptimizedPriorityGoalRow row2) {
            this.row = row2;
        }
        ...
        private final boolean alreadyInGoal() {
            for (int i = 0; i < OptimizedPriorityGoalRow.this.numGoals; i++) {
                if (OptimizedPriorityGoalRow.this.arrayGoals[i] == this.variable) {
                    return true;
                }
            }
            return false;
        ...
        public int compareTo(Object o) {
            return this.variable.f32id - ((SolverVariable) o).f32id;
        }
    ...
    public void updateFromRow(ArrayRow definition, boolean removeFromDefinition) {
        ...
        if (goalVariable != null) {
            ...
            while (current != -1 && 0 < currentSize) {
                int id = definition.variables.getId(current);
                float value = definition.variables.getValue(current);
                SolverVariable solverVariable = this.mCache.mIndexedVariables[id];
                this.accessor.init(solverVariable);
                if (this.accessor.addToGoal(goalVariable, value)) {
                    addToGoal(solverVariable);
                }
                this.constantValue += definition.constantValue * value;
                current = definition.variables.getNextIndice(current);
            }

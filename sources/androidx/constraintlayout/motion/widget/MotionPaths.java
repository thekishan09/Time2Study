...
class MotionPaths implements Comparable<MotionPaths> {
    ...
    public void initCartesian(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f5 = motionPaths.f30x;
        float position3 = position2;
        float position4 = motionPaths.f31y;
        float endCenterX = motionPaths2.f30x + (f / 2.0f);
        float endCenterY = motionPaths2.f31y + (f3 / 2.0f);
        float pathVectorX = endCenterX - (f5 + (f2 / 2.0f));
        float pathVectorY = endCenterY - (position4 + (f4 / 2.0f));
        this.f30x = (float) ((int) ((f5 + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f31y = (float) ((int) ((position4 + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f)));
        ...
        this.mMode = 2;
        this.f30x = (float) ((int) (((motionPaths.f30x + (pathVectorX * dxdx)) + (pathVectorY * dxdy)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f31y = (float) ((int) (((motionPaths.f31y + (pathVectorX * dydx)) + (pathVectorY * dydy)) - ((scaleY * scaleHeight) / 2.0f)));
    ...
    public void initScreen(int parentWidth, int parentHeight, KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f5 = motionPaths.f30x;
        float f6 = position2;
        float position3 = motionPaths.f31y;
        float endCenterX = motionPaths2.f30x + (f / 2.0f);
        float endCenterY = motionPaths2.f31y + (f3 / 2.0f);
        this.f30x = (float) ((int) ((f5 + ((endCenterX - (f5 + (f2 / 2.0f))) * path)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f31y = (float) ((int) ((position3 + ((endCenterY - (position3 + (f4 / 2.0f))) * path)) - ((scaleY * scaleHeight) / 2.0f)));
        this.width = (float) ((int) (f2 + (scaleX * scaleWidth)));
        this.height = (float) ((int) (f4 + (scaleY * scaleHeight)));
        this.mMode = 3;
        if (!Float.isNaN(keyPosition.mPercentX)) {
            this.f30x = (float) ((int) (keyPosition.mPercentX * ((float) ((int) (((float) parentWidth) - this.width)))));
        }
        if (!Float.isNaN(keyPosition.mPercentY)) {
            this.f31y = (float) ((int) (keyPosition.mPercentY * ((float) ((int) (((float) parentHeight) - this.height)))));
        }
        this.mKeyFrameEasing = Easing.getInterpolator(keyPosition.mTransitionEasing);
        this.mPathMotionArc = keyPosition.mPathMotionArc;
    }
    ...
    public void initPath(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f = motionPaths.f30x;
        float f2 = motionPaths.width;
        float f3 = motionPaths.f31y;
        float f4 = position2;
        float position3 = motionPaths.height;
        float endCenterX = motionPaths2.f30x + (motionPaths2.width / 2.0f);
        float endCenterY = motionPaths2.f31y + (motionPaths2.height / 2.0f);
        float pathVectorX = endCenterX - ((f2 / 2.0f) + f);
        float pathVectorY = endCenterY - (f3 + (position3 / 2.0f));
        this.f30x = (float) ((int) ((f + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f31y = (float) ((int) ((f3 + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f)));
        this.width = (float) ((int) (f2 + (scaleX * scaleWidth)));
        ...
        float f6 = (float) ((int) ((motionPaths3.f30x + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f30x = f6;
        float f7 = pathVectorX;
        float f8 = (float) ((int) ((motionPaths3.f31y + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f)));
        this.f31y = f8;
        this.f30x = f6 + ((-pathVectorY) * perpendicular);
        this.f31y = f8 + (pathVectorX * perpendicular);
    ...
    public void different(MotionPaths points, boolean[] mask, String[] custom, boolean arcMode) {
        ...
        int c2 = c + 1;
        mask[c] = mask[c] | diff(this.f30x, points.f30x) | arcMode;
        int c3 = c2 + 1;
        mask[c2] = mask[c2] | diff(this.f31y, points.f31y) | arcMode;
        int c4 = c3 + 1;
    ...
    public void fillStandard(double[] data, int[] toUse) {
        float[] set = {this.position, this.f30x, this.f31y, this.width, this.height, this.mPathRotate};
        int c = 0;
        for (int i = 0; i < toUse.length; i++) {

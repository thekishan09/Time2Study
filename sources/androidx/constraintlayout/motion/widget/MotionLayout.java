...
public class MotionLayout extends ConstraintLayout implements NestedScrollingParent3 {
    ...
    public void getViewVelocity(View view, float posOnViewX, float posOnViewY, float[] returnVelocity, int type) {
        ...
        if (this.mInterpolator != null) {
            ...
            v = (dir * ((interpos - position3) / 1.0E-5f)) / this.mTransitionDuration;
            position = position3;
        } else {
            position = position2;
        }
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if ((recalc || this.mModel.isNotConfiguredWith(startId, endId)) && this.mBeginState != -1) {
            ...
            if (this.mWidthMeasureMode == Integer.MIN_VALUE) {
                int i = this.mStartWrapWidth;
                androidLayoutWidth = (int) (((float) i) + (this.mPostInterpolationPosition * ((float) (this.mEndWrapWidth - i))));
                requestLayout();
            }
            if (this.mHeightMeasureMode == Integer.MIN_VALUE) {
                int i2 = this.mStartWrapHeight;
                androidLayoutHeight = (int) (((float) i2) + (this.mPostInterpolationPosition * ((float) (this.mEndWrapHeight - i2))));
                requestLayout();
            }
    ...
    public void onNestedPreScroll(View target, int dx, int dy, int[] consumed, int type) {
        ...
        if (motionScene != null && motionScene.mCurrentTransition != null && this.mScene.mCurrentTransition.isEnabled()) {
            ...
            if (currentTransition == null || !currentTransition.isEnabled() || (touchResponse = currentTransition.getTouchResponse()) == null || (regionId = touchResponse.getTouchRegionId()) == -1 || this.mScrollTarget.getId() == regionId) {
    ...
    public void evaluate(boolean force) {
        ...
        if (this.mKeepAnimating || (this.mInTransition && (force || this.mTransitionGoalPosition != this.mTransitionLastPosition))) {
            ...
            if (position <= 0.0f && (i = this.mBeginState) != -1) {
        ...
        if (Build.VERSION.SDK_INT >= 18 && newState && !isInLayout()) {
            requestLayout();
        }
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs, C0216R.styleable.MotionLayout);
            int N = a.getIndexCount();
            boolean apply = true;
            for (int i = 0; i < N; i++) {
                int attr = a.getIndex(i);
                if (attr == C0216R.styleable.MotionLayout_layoutDescription) {
                    this.mScene = new MotionScene(getContext(), this, a.getResourceId(attr, -1));
                } else if (attr == C0216R.styleable.MotionLayout_currentState) {
                    this.mCurrentState = a.getResourceId(attr, -1);
                } else if (attr == C0216R.styleable.MotionLayout_motionProgress) {
                    this.mTransitionGoalPosition = a.getFloat(attr, 0.0f);
                    this.mInTransition = true;
                } else if (attr == C0216R.styleable.MotionLayout_applyMotionScene) {
                    apply = a.getBoolean(attr, apply);
                } else {
                    int i2 = 0;
                    if (attr == C0216R.styleable.MotionLayout_showPaths) {
                    ...
                    } else if (attr == C0216R.styleable.MotionLayout_motionDebug) {

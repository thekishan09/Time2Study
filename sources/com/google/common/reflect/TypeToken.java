...
public abstract class TypeToken<T> extends TypeCapture<T> implements Serializable {
    ...
    /* renamed from: of */
    public static <T> TypeToken<T> m567of(Class<T> type) {
        return new SimpleTypeToken(type);
    }

    /* renamed from: of */
    public static TypeToken<?> m568of(Type type) {
        return new SimpleTypeToken(type);
    }
    ...
    public final <X> TypeToken<T> where(TypeParameter<X> typeParam, TypeToken<X> typeArg) {
        return new SimpleTypeToken(new TypeResolver().where(ImmutableMap.m447of(new TypeResolver.TypeVariableKey(typeParam.typeVariable), typeArg.runtimeType)).resolveType(this.runtimeType));
    }

    public final <X> TypeToken<T> where(TypeParameter<X> typeParam, Class<X> typeArg) {
        return where(typeParam, m567of(typeArg));
    }

    public final TypeToken<?> resolveType(Type type) {
        Preconditions.checkNotNull(type);
        return m568of(getInvariantTypeResolver().resolveType(type));
    }

    private TypeToken<?> resolveSupertype(Type type) {
        TypeToken<?> supertype = m568of(getCovariantTypeResolver().resolveType(type));
    ...
    public final TypeToken<? extends T> getSubtype(Class<?> subclass) {
        ...
        TypeToken<?> of = m568of(resolveTypeArgsForSubclass(subclass));
    ...
    public final boolean isSupertypeOf(Type type) {
        return m568of(type).isSubtypeOf(getType());
    }
    ...
    public final boolean isSubtypeOf(Type supertype) {
        ...
        if (type instanceof TypeVariable) {
            ...
            return m568of(supertype).isSupertypeOfArray((GenericArrayType) this.runtimeType);
        } else {
    ...
    public final TypeToken<T> wrap() {
        if (isPrimitive()) {
            return m567of(Primitives.wrap((Class) this.runtimeType));
        }
        return this;
    }
    ...
    public final TypeToken<T> unwrap() {
        if (isWrapper()) {
            return m567of(Primitives.unwrap((Class) this.runtimeType));
        }
        return this;
    }
    ...
    public final Invokable<T, T> constructor(Constructor<?> constructor) {
        ...
        return new Invokable.ConstructorInvokable<T>(constructor) {
            ...
            public String toString() {
                return getOwnerType() + "(" + Joiner.m397on(", ").join((Object[]) getGenericParameterTypes()) + ")";
            }
        };
    }
    ...
    public Object writeReplace() {
        return m568of(new TypeResolver().resolveType(this.runtimeType));
    }
    ...
    private boolean isSubtypeOfParameterizedType(ParameterizedType supertype) {
        Class<? super Object> rawType = m568of((Type) supertype).getRawType();
        ...
        for (int i = 0; i < typeVars.length; i++) {
            if (!m568of(getCovariantTypeResolver().resolveType(typeVars[i])).m566is(supertypeArgs[i], typeVars[i])) {
                return false;
            }
        }
    ...
    private boolean isSubtypeOfArrayType(GenericArrayType supertype) {
        ...
        if (type instanceof Class) {
            ...
            return m567of(fromClass.getComponentType()).isSubtypeOf(supertype.getGenericComponentType());
        } else if (type instanceof GenericArrayType) {
            return m568of(((GenericArrayType) type).getGenericComponentType()).isSubtypeOf(supertype.getGenericComponentType());
        } else {
            return false;
        }
    }
    ...
    private boolean isSupertypeOfArray(GenericArrayType subtype) {
        ...
        if (type instanceof Class) {
            ...
            return m568of(subtype.getGenericComponentType()).isSubtypeOf((Type) thisClass.getComponentType());
        } else if (type instanceof GenericArrayType) {
            return m568of(subtype.getGenericComponentType()).isSubtypeOf(((GenericArrayType) this.runtimeType).getGenericComponentType());
        } else {
            return false;
        }
    }
    ...
    /* renamed from: is */
    private boolean m566is(Type formalType, TypeVariable<?> declaration) {
    ...
    private static class Bounds {
        ...
        public boolean isSubtypeOf(Type supertype) {
            for (Type bound : this.bounds) {
                boolean isSubtypeOf = TypeToken.m568of(bound).isSubtypeOf(supertype);
        ...
        public boolean isSupertypeOf(Type subtype) {
            TypeToken<?> type = TypeToken.m568of(subtype);
            for (Type bound : this.bounds) {
    ...
    public ImmutableSet<Class<? super T>> getRawTypes() {
        ...
        new TypeVisitor() {
            ...
            public void visitGenericArrayType(GenericArrayType t) {
                builder.add((Object) Types.getArrayClass(TypeToken.m568of(t.getGenericComponentType()).getRawType()));
            }
        }.visit(this.runtimeType);
        return builder.build();
    }
    ...
    private boolean isOwnedBySubtypeOf(Type supertype) {
        ...
        while (it.hasNext()) {
            ...
            if (ownerType != null && m568of(ownerType).isSubtypeOf(supertype)) {
                return true;
            }
        }
        return false;
    ...
    static <T> TypeToken<? extends T> toGenericType(Class<T> cls) {
        if (cls.isArray()) {
            return m568of(Types.newArrayType(toGenericType(cls.getComponentType()).runtimeType));
        }
        ...
        if (typeParams.length > 0 || (ownerType != null && ownerType != cls.getEnclosingClass())) {
            return m568of((Type) Types.newParameterizedTypeWithOwner(ownerType, cls, typeParams));
        }
        return m567of(cls);
    }
    ...
    private TypeToken<? super T> getSupertypeFromUpperBounds(Class<? super T> supertype, Type[] upperBounds) {
        for (Type upperBound : upperBounds) {
            TypeToken<?> of = m568of(upperBound);
    ...
    private TypeToken<? extends T> getSubtypeFromLowerBounds(Class<?> subclass, Type[] lowerBounds) {
        if (lowerBounds.length > 0) {
            return m568of(lowerBounds[0]).getSubtype(subclass);
        }
        throw new IllegalArgumentException(subclass + " isn't a subclass of " + this);
    }

    private TypeToken<? super T> getArraySupertype(Class<? super T> supertype) {
        return m568of(newArrayClassOrGenericArrayType(((TypeToken) Preconditions.checkNotNull(getComponentType(), "%s isn't a super type of %s", (Object) supertype, (Object) this)).getSupertype(supertype.getComponentType()).runtimeType));
    }

    private TypeToken<? extends T> getArraySubtype(Class<?> subclass) {
        return m568of(newArrayClassOrGenericArrayType(getComponentType().getSubtype(subclass.getComponentType()).runtimeType));
    }
    ...
    private static abstract class TypeCollector<K> {
        ...
        public final ImmutableList<K> collectTypes(K type) {
            return collectTypes(ImmutableList.m428of(type));
        }

...
public final class OkHeaders {
    ...
    public static void addCookies(Request.Builder builder, Map<String, List<String>> cookieHeaders) {
        for (Map.Entry<String, List<String>> entry : cookieHeaders.entrySet()) {
            ...
            if ((HttpHeaders.COOKIE.equalsIgnoreCase(key) || "Cookie2".equalsIgnoreCase(key)) && !entry.getValue().isEmpty()) {
    ...
    public static Set<String> varyFields(Headers responseHeaders) {
        ...
        for (int i = 0; i < size; i++) {
            if (HttpHeaders.VARY.equalsIgnoreCase(responseHeaders.name(i))) {
    ...
    static boolean isEndToEnd(String fieldName) {
        return !HttpHeaders.CONNECTION.equalsIgnoreCase(fieldName) && !"Keep-Alive".equalsIgnoreCase(fieldName) && !HttpHeaders.PROXY_AUTHENTICATE.equalsIgnoreCase(fieldName) && !HttpHeaders.PROXY_AUTHORIZATION.equalsIgnoreCase(fieldName) && !HttpHeaders.f396TE.equalsIgnoreCase(fieldName) && !"Trailers".equalsIgnoreCase(fieldName) && !HttpHeaders.TRANSFER_ENCODING.equalsIgnoreCase(fieldName) && !HttpHeaders.UPGRADE.equalsIgnoreCase(fieldName);
    }
        ...
        for (int i = 0; i < size; i++) {
            if (challengeHeader.equalsIgnoreCase(responseHeaders.name(i))) {
                ...
                while (realmStart < value.length()) {
                    ...
                    int pos = HeaderParser.skipUntil(value, realmStart, " ");
